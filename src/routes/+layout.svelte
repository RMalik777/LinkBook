...
<ModeWatcher />
<div
	class="flex h-fit min-h-dvh w-full max-w-full flex-col items-center justify-between scroll-smooth bg-white bg-radial from-purple-100 from-[2px] to-0% bg-[size:50px_50px] bg-fixed antialiased sm:from-[3px] sm:bg-[size:60px_60px] dark:bg-neutral-950 dark:from-purple-950"
>
	<button
		type="button"
		...
		onclick={() => {
			if ($userPrefersMode === "light") setMode("dark");
			else if ($userPrefersMode === "dark") setMode("system");
			else setMode("light");
		}}
	>
		<Moon
			class={clsx(
				$userPrefersMode === "dark" ? "scale-100 rotate-0" : "scale-0 rotate-90",
				" relative h-4 duration-300 ease-out sm:h-5 md:h-6",
			)}
		/>
		<Sun
			class={clsx(
				$userPrefersMode === "light" ? "scale-100 rotate-0" : "scale-0 rotate-90",
				" absolute h-4 duration-300 ease-out sm:h-5 md:h-6",
			)}
		/>
		<SunMoon
			class={clsx(
				$userPrefersMode !== "light" && $userPrefersMode !== "dark"
					? "scale-100 rotate-0"
					: "scale-0 -rotate-90",
				" absolute h-4 duration-300 ease-out sm:h-5 md:h-6",
			)}
		/>
		<span class="duration-200 ease-out">{$userPrefersMode ? $userPrefersMode : "System"}</span
		></button
	>
